...
module.exports = (robot) ->
  ...
  robot.respond /remind me in ((?:(?:\d+) (?:weeks?|days?|hours?|hrs?|minutes?|mins?|seconds?|secs?)[ ,]*(?:and)? +)+)(to|that) (.*)/i, (msg) ->
    time = msg.match[1]
    action = msg.match[3]
  ...
  robot.respond /remind me (at|on) (((?!(to |that )).)*) (to|that) (.*)/i, (msg) ->
    ...
    if parsedDate.isValid() and parsedDate.isFuture()
      ...
      msg.send 'Could not parse given date, see http://sugarjs.com/dates for examples of valid dates'
